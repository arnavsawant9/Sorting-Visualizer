// src/components/ShellSort.jsx
import React, { useState } from "react";
import { ALGORITHM_INFO } from "../algorithms/algorithmInfo";
import java from "../assets/java.png";
import cpp from "../assets/cpp.png";
import javascript from "../assets/javascript.png";
import { javascript_code, java_code, cpp_code } from "../code/shellSort";

export default function ShellSort() {
  const info = ALGORITHM_INFO.shell;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto mt-10 p-8 bg-white rounded-3xl shadow-xl border border-blue-200">
      <h2 className="text-4xl font-bold mb-6 text-center text-white bg-gradient-to-r from-orange-500 to-yellow-500 rounded-xl py-4 shadow-md tracking-wide">
        {info.name}
      </h2>

      <div className="flex flex-col md:flex-row gap-8">
        <div className="md:w-2/3 bg-yellow-50 text-gray-800 p-6 rounded-2xl shadow-sm leading-relaxed space-y-4 text-justify">
          {info.description.map((para, i) => (
            <p key={i}>{para}</p>
          ))}
        </div>

        <div className="md:w-1/3 bg-yellow-100 text-yellow-900 rounded-2xl p-6 shadow-sm text-base space-y-5 font-medium">
          <div>
      ...
      <div className="mt-10">
        ...
        <pre className="bg-gray-900 text-yellow-200 rounded-xl p-6 overflow-x-auto text-sm font-mono shadow-inner">
          {code}
        </pre>
